...
const Timer: React.FC<TimerProps> = ({ time, isActive, playerName }) => (
  <div
    className={`p-3 rounded-lg text-center w-48 ${
      isActive ? "bg-green-500 text-white" : "bg-gray-600 text-gray-200"
    }`}
  >
    <div className="text-sm font-bold">{playerName}</div>
    <div className="font-mono text-3xl font-semibold tracking-wider">
      {formatTime(time)}
    </div>
  </div>
);
...
const GamePage: React.FC<GamePageProps> = ({ gameMode, roomId, onExit }) => {
  ...
  useEffect(() => {
    ...
    if (gameMode === "random") {
      ...
      url = `${base}/play-with-friend/${roomId}?cid=${encodeURIComponent(
        clientId
      )}`;
    }
    if (!url) return;
    ...
    socket.onopen = () => {
      if (gameMode === "random")
        socket.send(JSON.stringify({ type: "queueRandom", clientId }));
      else if (gameMode === "friend" && roomId != null)
        socket.send(JSON.stringify({ type: "joinRoom", roomId, clientId }));
    };
      ...
      try {
        ...
        switch (message.type) {
          ...
          case "matchFound": {
            ...
            if (
              message.isSetter === true ||
              (message.isSetter === undefined && color === "white")
            ) {
          ...
          case "chatMessage":
            setMessages((prev) => [
              ...prev,
              { sender: "Opponent", text: message.text },
            ]);
            break;
          case "gameOverOnTime":
            updateGameStatus();
  ...
  const squareStyles = useMemo(() => {
    ...
    if (lastMove) {
      styles[lastMove.from] = {
        ...styles[lastMove.from],
        backgroundColor: "rgba(204, 222, 0, 0.4)",
      };
      styles[lastMove.to] = {
        ...styles[lastMove.to],
        backgroundColor: "rgba(204, 222, 0, 0.4)",
  ...
  const handleTimeSelection = () => {
    const timeInMinutes =
      selectedTime === "null" ? null : parseInt(selectedTime, 10);
    ...
    if (ws) {
      ws.send(
        JSON.stringify({ type: "startGame", timeControl: timeInSeconds })
      );
    }
  };
        ...
        <div className="bg-gray-800 p-8 rounded-lg shadow-xl border border-gray-700 w-full max-w-xs">
          ...
          <div className="flex flex-col gap-4">
            ...
            <button
              ...
              className="p-3 bg-green-600 hover:bg-green-700 rounded-lg transition font-bold"
            >
              Start Game
            </button>
          </div>
          <button
            onClick={onExit}
            className="w-full mt-6 bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-4 rounded-lg transition duration-300"
          >
            Back to Menu
          </button>
        </div>
  ...
  return (
    <div className="flex items-center justify-center min-h-screen w-full p-2 md:p-4 bg-gray-900 text-white">
      <div className="w-full max-w-7xl mx-auto flex flex-col lg:flex-row items-start justify-center gap-6">
        <div className="w-full lg:w-3/5 order-1 flex flex-col items-center gap-4">
          <div className="w-full flex justify-center items-center gap-4">
            {timeControl !== null && (
              <Timer
                time={playerColor === "white" ? blackTime : whiteTime}
                isActive={game.turn() !== playerColor[0]}
                playerName="Opponent"
              />
            )}
            {timeControl !== null && (
          ...
          <div className="w-full max-w-lg mx-auto relative">
            <Chessboard
              options={{
                ...
                onSquareClick,
                boardOrientation,
                squareStyle: { backgroundColor: "#a2d2ff" },
                darkSquareStyle: { backgroundColor: "#0077b6" },
                squareStyles,
              }}
            />
            {showPromotionDialog && (
